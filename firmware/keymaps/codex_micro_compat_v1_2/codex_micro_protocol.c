...
static const uint8_t task_led_indices[CODEX_MICRO_TASK_KEY_COUNT] = {0, 1, 4, 5, 8, 9};
...
static void apply_global_lighting(uint8_t brightness, bool off) {
    ...
    lighting_off      = off;
    rgb_matrix_sethsv_noeeprom(rgb_matrix_get_hue(), rgb_matrix_get_sat(), off ? 0 : brightness);
}
...
bool codex_micro_rgb_indicators(uint8_t led_min, uint8_t led_max) {
    ...
    for (uint8_t slot = 0; slot < CODEX_MICRO_TASK_KEY_COUNT; ++slot) {
        uint8_t led = task_led_indices[slot];
        if (led < led_min || led >= led_max) {
            continue;
        }
        ...
        if (host_seen && light->assigned && light->effect != CODEX_MICRO_EFFECT_OFF) {
            ...
            uint8_t source_red   = (uint8_t)((light->color >> 16) & 0xFFU);
            uint8_t source_green = (uint8_t)((light->color >> 8) & 0xFFU);
            uint8_t source_blue  = (uint8_t)(light->color & 0xFFU);
